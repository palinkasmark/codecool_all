...
def move_player(player_mark):
    while True:
        board = get_board()
        p_input = int(input(f"player { player_mark } move [from 0 to 6]: "))
        if p_input < len(board) and p_input > 0:
            board[p_input] = player_mark
            print(board)
            break
        else:
            print("Invalid input, please try again: ")
...
# You will need a few arguments;)
""" def move_AI(player_mark, board_id = 1, dot="."):
    while True:
        if board_id == 1: board = get_board1()
        if board_id == 2: board = get_board2()

        random_index = random.randint(0, len(board))

        if board[random_index] == dot:
            board[random_index] = player_mark
            print(board)
            break
        else:
            print("That cell is already occupied") """
